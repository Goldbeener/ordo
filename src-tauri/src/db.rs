...
impl NoteDatabase {
    ...
    pub fn list_notes_by_id(
        ...
        let query = if last_id.is_some() {
            format!(
                "SELECT * FROM notes WHERE id < ? ORDER BY create_time DESC LIMIT {}",
                page_size
            )
        } else {
            format!(
                "SELECT * FROM notes ORDER BY create_time DESC LIMIT {}",
                page_size
            )
        };
        ...
        // 定义一个闭包函数来处理行转换，避免代码重复
        let map_row = |row: &rusqlite::Row| -> Result<Note> {
            Ok(Note {
                id: Some(row.get(0)?),
                title: row.get(1)?,
                content: row.get(2)?,
        ...
        // 执行查询
        let note_iter = if let Some(id) = last_id {
